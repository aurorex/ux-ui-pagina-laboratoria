# Pagina Laboratoria:ux-ui

## Partes del ux

### **Informacion:**

#### El ux se encarga de seleccionar lo mas importante, la informacion en especifico que necesita mostrar el cliente y la informacion detallada que necesita saber el usuario.

 ![informacion](assets/img/informacion.png)
 ![laboratoria](assets/img/laboratoria.png)

### **arquitectura:**

####  la informacion, las imagenes y otros elementos se han distribuido de manera vertical para manejar la manera en que el usuario navegara por la pagina.

![arquitectura](assets/img/arquitectura.png)

### **clasifica:**

#### divide la informacion de acuerdo al usuario-postulante y al usuario-empresarial en dos opciones: estudiantes-empresas.

![clasifica](assets/img/clasifica.png)

### **acceso persistente a las redes sociales**

#### la disposición de iconos en todo momento en el que navega el usuario es persistente.si los iconos no permitesen visualizar parte del contenido de la pagina,el usuario tiene la opcion de desaparecerlos momentaneamente.en la sección "blogs" tambien persisten estos iconos pero de manera sugerida al colocar el pulsor en cada blog.

![redes](assets/img/redes.png)
![blog](assets/img/blog.png)

### **influenciar**

#### el ux en todo momento debe destacar que es lo que hace unico el producto o servicio y destacar sus virtudes,generando confianza en los usuarios. los reconocimientos,impacto y aliados son un ejemplo de ello.

![impacto](assets/img/impacto.png)
![reconocimientos](assets/img/reconocimientos.png)

### **testimonios**

#### al brindar testimonios o experiencias de personas que han  experimentado el servicio,genera en el usuario confianza,motivacion e identificarse con algunos hechos o vivencias que se comparte en la pagina.

![testimonio](assets/img/testimonio.png)

### **contacto directo**

#### por medio de un chat se le brinda al usuario la facilidad de poder contactar en tiempo real a la empresa y asi poder realizar cualquier consulta desde el lugar en el que se encuentre.

![chat](assets/img/chat.png)

### **registro**

#### al entrar a la seccion de *postulante* no solo veremos informacion mas detallada,tambien tendremos la opcion de registrarnos para poder pertenecer como postulante de laboratoria

![form](assets/img/form.png)

## Partes del UI:

![tipografia-color](assets/img/tipografia-color.png)

### **colores**

#### se decide que colores se van a definir tanto para la marca del producto,como a sus elementos(buttons,parrafos,enlaces,etc).

### **tipografia**

#### establecer el tipo de letra que se tiene para los títulos, parrafos y subtítulos, etc.

### **posicionamiento de contenido**

#### el espacio que van a ocupar los elementos de cada seccion(imagenes,parrafos,botones,etc).

![position](assets/img/position.png)

### **tamaño**

#### tamaños establecidos para parrafos,imagenes,titulos,divs,buttons.

### **diseño**

#### los bordes de las imagenes,el diseño para los iconos, el evento hover para los buttons, el diseño de subrayado para los titulos de cada sección.el diseño de logo para laboratoria.

![diseño](assets/img/diseño.png)
